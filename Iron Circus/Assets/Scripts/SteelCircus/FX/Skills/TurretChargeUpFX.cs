// Decompiled with JetBrains decompiler
// Type: SteelCircus.FX.Skills.TurretChargeUpFX
// Assembly: Assembly-CSharp, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 841B04D4-2E17-4B98-AC76-29D6D8A2480C
// Assembly location: D:\SteamLibrary\steamapps\common\Steel_Circus\sc_Data\Managed\Assembly-CSharp.dll

using Imi.SharedWithServer.Game.Skills;
using Imi.SteelCircus.ScriptableObjects;
using UnityEngine;

namespace SteelCircus.FX.Skills
{
  public class TurretChargeUpFX : MonoBehaviour, IVfx
  {
    private Material mat;
    private Color col;
    public float fadeInDuration = 0.2f;
    private float fadeInCounter;

    private void Awake() => this.mat = this.GetComponentInChildren<MeshRenderer>().material;

    private void Update()
    {
      this.fadeInCounter = Mathf.Clamp01(this.fadeInCounter + Time.deltaTime / this.fadeInDuration);
      this.UpdateColor();
    }

    public void SetOwner(GameEntity entity)
    {
      this.col = SingletonScriptableObject<ColorsConfig>.Instance.LightColor(entity.playerTeam.value);
      this.UpdateColor();
    }

    private void UpdateColor() => this.mat.color = this.col * this.fadeInCounter;

    public void SetArgs(object args)
    {
    }

    public void SetSkillGraph(SkillGraph graph)
    {
    }
  }
}
