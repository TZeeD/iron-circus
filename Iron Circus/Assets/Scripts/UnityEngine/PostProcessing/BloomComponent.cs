...
namespace UnityEngine.PostProcessing
{
  public sealed class BloomComponent : PostProcessingComponentRenderTexture<BloomModel>
  {
    ...
    public void Prepare(RenderTexture source, Material uberMaterial, Texture autoExposure)
    {
      ...
      RenderTexture renderTexture1 = this.context.renderTextureFactory.Get(width, height, format: format);
      Graphics.Blit((Texture) source, renderTexture1, mat, 0);
      RenderTexture renderTexture2 = renderTexture1;
      for (int index = 0; index < num3; ++index)
      {
        this.m_BlurBuffer1[index] = this.context.renderTextureFactory.Get(renderTexture2.width / 2, renderTexture2.height / 2, format: format);
      ...
      for (int index = num3 - 2; index >= 0; --index)
      {
        ...
        this.m_BlurBuffer2[index] = this.context.renderTextureFactory.Get(renderTexture3.width, renderTexture3.height, format: format);
