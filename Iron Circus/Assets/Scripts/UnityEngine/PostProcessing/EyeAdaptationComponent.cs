    ...
    PostProcessingComponentRenderTexture<EyeAdaptationModel>
  {
    ...
    public Texture Prepare(RenderTexture source, Material uberMaterial)
    {
      ...
      else
      {
        ...
        int num2;
        RenderTexture renderTexture3 = this.m_AutoExposurePool[(num2 = num1 + 1) % 2];
        RenderTexture dest = renderTexture3;
        Material mat2 = mat1;
        int adaptationType = (int) settings.adaptationType;
        Graphics.Blit((Texture) renderTexture2, dest, mat2, adaptationType);
        int num3;
      ...
      if (this.context.profile.debugViews.IsModeActive(BuiltinDebugViewsModel.Mode.EyeAdaptation))
      {
        ...
        {
          RenderTexture renderTexture4 = new RenderTexture(256, 128, 0, RenderTextureFormat.ARGB32);
          renderTexture4.filterMode = FilterMode.Point;
          renderTexture4.wrapMode = TextureWrapMode.Clamp;
          this.m_DebugHistogram = renderTexture4;
        }
