// Decompiled with JetBrains decompiler
// Type: Imi.SharedWithServer.Networking.Messages.MineTriggeredMessage
// Assembly: Assembly-CSharp, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 841B04D4-2E17-4B98-AC76-29D6D8A2480C
// Assembly location: D:\SteamLibrary\steamapps\common\Steel_Circus\sc_Data\Managed\Assembly-CSharp.dll

using Imi.SharedWithServer.Networking.Messages.SerDes;
using Jitter.LinearMath;

namespace Imi.SharedWithServer.Networking.Messages
{
  public class MineTriggeredMessage : Message
  {
    public int mineId;
    public JVector minePosition;
    public ulong playerId;
    public float timeToDetonation;

    public MineTriggeredMessage()
      : base(RumpfieldMessageType.MineTriggered)
    {
    }

    public MineTriggeredMessage(
      ulong playerId,
      int mineId,
      JVector minePosition,
      float timeToDetonation)
      : base(RumpfieldMessageType.MineTriggered)
    {
      this.playerId = playerId;
      this.mineId = mineId;
      this.minePosition = minePosition;
      this.timeToDetonation = timeToDetonation;
    }

    protected override void SerializeOrDeserialize(IMessageSerDes messageSerDes)
    {
      messageSerDes.ULong(ref this.playerId);
      messageSerDes.Int(ref this.mineId);
      messageSerDes.Float(ref this.minePosition.X);
      messageSerDes.Float(ref this.minePosition.X);
      messageSerDes.Float(ref this.minePosition.X);
      messageSerDes.Float(ref this.timeToDetonation);
    }
  }
}
