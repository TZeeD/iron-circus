...
// Type: Imi.SharedWithServer.Game.Skills.ApplySpeedModToOwnerState
...
{
  public class ApplySpeedModToOwnerState : SkillState
  {
    [SyncValue]
    public SyncableValue<float> modifier;
    [SyncValue]
    ...
    {
      this.modifier.Parse(serializationInfo, ref valueIndex, this.Name + ".modifier");
    ...
    {
      this.modifier.Serialize(target, ref valueIndex);
    ...
    {
      this.modifier.Deserialize(target, ref valueIndex);
    ...
    {
      this.stop = this.skillGraph.AddVar<bool>("[ApplySpeedModToOwnerState][" + this.name + "]Internal");
    ...
    protected override void EnterDerived()
    {
      ...
      if ((double) this.duration.Get() <= 0.0)
        this.skillGraph.GetOwner().AddStatusEffect(this.skillGraph.GetContext(), StatusEffect.MovementSpeedChange(this.skillGraph.GetOwnerId(), this.modifier.Get(), 1000f, this.stop));
      else
        this.skillGraph.GetOwner().AddStatusEffect(this.skillGraph.GetContext(), StatusEffect.MovementSpeedChange(this.skillGraph.GetOwnerId(), this.modifier.Get(), this.duration.Get(), this.stop));
    }
