Shader "SteelCircus/FX/Skills/AoePreviews/AoeBarrierShader" {
	Properties {
		[NoScaleOffset] _MaskTex ("Mask. R = Outline, G = Alpha, B = Outline Position", 2D) = "white" {}
		[NoScaleOffset] _GradientTex ("Gradient", 2D) = "white" {}
		_ColorMiddle ("Mid Color", Vector) = (1,0.5,0,1)
		_OutlineSpeed ("Outline Speed", Float) = 1
		_MainTex ("Tiling Texture", 2D) = "white" {}
		_ColorDark ("Dark Color (Additive for tiling)", Vector) = (1,0.5,0,1)
		[NoScaleOffset] _TileAnimationTex ("Tile Animation Tex (cols = tile intensity, rows = anim data)", 2D) = "white" {}
		_TileAnimationSpeed ("Tile Animation Speed", Float) = 1
		...
		Pass {
			...
			Program "fp" {
				SubProgram "d3d11 hw_tier00 " {
					...
					uniform  sampler2D _TileAnimationTex;
					...
					void main()
					{
					    u_xlat0.y = _TileAnimationSpeed * _Time.y;
					    u_xlat1 = texture(_MainTex, vs_TEXCOORD1.xy);
					    u_xlat0.x = u_xlat1.x + 0.0454545468;
					    u_xlat0 = texture(_TileAnimationTex, u_xlat0.xy);
					    u_xlat0.xyz = u_xlat1.yyy * u_xlat0.xyz;
					    u_xlat0.xyz = u_xlat0.xyz * _ColorDark.xyz;
					    u_xlat1 = texture(_GradientTex, vs_TEXCOORD0.xy);
					    u_xlat0.xyz = u_xlat1.xyz * _ColorMiddle.xyz + u_xlat0.xyz;
					    ...
					    u_xlat1.x = _Time.y * _OutlineSpeed + u_xlat3.z;
					    u_xlat1.y = 0.0;
