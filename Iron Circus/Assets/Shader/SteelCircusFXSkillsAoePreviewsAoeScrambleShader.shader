Shader "SteelCircus/FX/Skills/AoePreviews/AoeScrambleShader" {
	Properties {
		_MainTex ("Texture (Tiling)", 2D) = "white" {}
		_DispTex ("Displacement Texture (Tiling)", 2D) = "white" {}
		_DispStrength ("Displacement intensity", Float) = 0.3
		_DispScrollSpeed ("Displacement scroll speed", Float) = 0.3
		...
		_UVFadeStart ("UV.y fade start", Range(0, 1)) = 0.5
		_UVFadeEnd ("UV.y fade end", Range(0, 1)) = 1
		_FillTextureUVPow ("UV pow for fill texture", Float) = 0.5
			...
			Program "fp" {
				SubProgram "d3d11 hw_tier00 " {
					...
					void main()
					{
					    ...
					    u_xlat0.x = u_xlat0.x * _FillTextureUVPow;
					    u_xlat0.x = exp2(u_xlat0.x);
					    u_xlat0.y = u_xlat0.x * _MainTex_ST.y;
					    u_xlat8.xy = vs_TEXCOORD0.xy * _DispTex_ST.xy + _DispTex_ST.zw;
					    u_xlat1.y = _DispScrollSpeed * _Time.y;
					    u_xlat1.x = float(0.0);
					    u_xlat9.x = float(0.0);
					    u_xlat8.xy = u_xlat8.xy + u_xlat1.xy;
					    u_xlat2 = texture(_DispTex, u_xlat8.xy);
					    u_xlat9.y = u_xlat2.y;
					    u_xlat0.x = vs_TEXCOORD0.x;
					    u_xlat0.xy = u_xlat9.xy * vec2(vec2(_DispStrength, _DispStrength)) + u_xlat0.xy;
					    u_xlat1.y = (-_ScrollSpeed) * _Time.y;
					    u_xlat1.x = float(0.0);
					    u_xlat9.y = float(0.0);
					    u_xlat0.xy = u_xlat0.xy + u_xlat1.xy;
					    u_xlat0 = texture(_MainTex, u_xlat0.xy);
					    u_xlat0.xyz = u_xlat0.xyz * _ColorMiddle.xyz;
					    u_xlat2.xyz = _ColorBuildupBG.xyz;
					    u_xlat3 = texture(_MaskTex, vs_TEXCOORD0.xy);
					    u_xlat2.w = u_xlat3.y * _ColorBuildupBG.w;
					    u_xlat0.w = u_xlat3.y;
					    u_xlat2 = (-u_xlat0) + u_xlat2;
					    u_xlatb1 = _BuildupBGTime>=vs_TEXCOORD0.y;
					    u_xlat1.x = (u_xlatb1) ? 0.0 : 1.0;
					    u_xlat0 = u_xlat1.xxxx * u_xlat2 + u_xlat0;
					    u_xlat9.x = _Time.y * _OutlineSpeed + u_xlat3.z;
					    u_xlat1 = texture(_OutlineMaskTex, u_xlat9.xy);
					    u_xlat2 = (-_ColorOutlineBG) + _ColorOutlineFG;
					    u_xlat1 = u_xlat1.xxxx * u_xlat2 + _ColorOutlineBG;
					    u_xlat1.xyz = (-u_xlat0.xyz) + u_xlat1.xyz;
					    u_xlat13 = u_xlat1.w * u_xlat3.x;
					    u_xlat13 = u_xlat13 * _OutlineStrength;
					    u_xlat0.xyz = vec3(u_xlat13) * u_xlat1.xyz + u_xlat0.xyz;
					    u_xlat1.w = u_xlat0.w * _ColorBuildupBG.w;
					    u_xlat1.xyz = _ColorBuildupBG.xyz;
					    ...
					    u_xlat1.x = vs_TEXCOORD0.y + (-_UVFadeStart);
					    u_xlat5 = (-_UVFadeStart) + _UVFadeEnd;
